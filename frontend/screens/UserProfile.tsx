  ...
  useEffect(() => {
    ...
    const getOrg = async() => {
        try{
            ...
            const res = await axios.get(`${API_BASE_URL}org?orgAbbr=${orgAbbr.organization.orgAbbr}`);
    ...
    const getUser= async() => {
        try{
            ...
            const fetchUser = await axios.get(`${API_BASE_URL}user?email=${user.email}`);
      ...
      <SafeAreaView className="flex-1">
        <ScrollView contentContainerStyle={{ alignItems: 'center', paddingBottom: 2 }}>
          ...
          <View className="items-center">
            ...
            />
            <Text className="color-white font-bold text-xl">{user?.userName}</Text>
            <Text className="color-white font-thin">{user?.email}</Text>
